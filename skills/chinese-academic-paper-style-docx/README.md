...
>
> - Agent 使用指令请看 **`SKILL.md`**（含 Markdown 书写约定、三条工作流、格式速查）。
> - 上游完整正文（XML 参考 / 11 个问题的解法）见 **`REFERENCE.md`**。
> - 本仓库适配：脚本路径改为相对技能目录、补充依赖与版本要求、修复
>   `table.py` / `formula.py` 生成 XML 的 OOXML 元素顺序（schema 校验）问题。
> - 下文为上游 README 原文（快速开始部分已按本技能用法更新）。
...
python3 -m pip install -r "${SKILL_DIR}/requirements.txt"   # 解包/校验脚本用

# 2️⃣ 按约定写 Markdown 论文（标题/公式/三线表/图/引用约定见 SKILL.md）
