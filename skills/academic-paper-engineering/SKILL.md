...
> 下面所有路径（`references/…`、`scripts/…`、`assets/templates/…`）都以该目录为基准 ——
> **先 `cd "<技能目录>"` 再执行命令**。
>
> **约束**：工作区之外的文件不能用 read_file / 编辑器读写（会被安全策略拒绝：`项目外路径未获授权`）。
> 需要查看随技能附带的参考文档或模板时，用 `cat "references/xxx.md"`、`cat "assets/templates/CHEATSHEET.md"`
> （同样先 cd 进技能目录）。
>
> **动手前先跑环境自检**：
>
...
> **不要当作失败、不要报错中止** —— 这是预期行为：LaTeX 工程已经生成且可用。
> 此时应把返回里的 `guidance` 内容转达给用户（本地装 TeX 后自行编译，或把工程上传 Overleaf），
> 并明确告知「工程已交付、仅 PDF 未编译」。
