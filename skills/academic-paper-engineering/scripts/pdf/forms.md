...
To visually verify field bounding boxes on the original PDF, create a validation overlay image:
`python scripts/create_validation_image.py <input.pdf> <field_info.json> <output_dir/>`

This draws red boxes around fillable field entry areas and blue boxes around field labels, making it easy to confirm that field coordinates are correct before filling.
