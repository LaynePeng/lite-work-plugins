...
```bash
python scripts/xlsx/recalc.py output.xlsx [timeout_seconds]   # default 30
```
...
`openpyxl`, `pandas`, `markitdown` (pip, preinstalled - install only if an import fails or the command is missing) · LibreOffice (`soffice`, auto-configured for sandboxed environments via `scripts/common/office/soffice.py`)
