...
def render_claim_tree_markdown(
    ...
    *,
    ...
    for branch, num, delta in rows:
        # 反斜杠不能出现在 f-string 表达式内（Python 3.12 起才允许，PEP 701）——
        # 先算好再插值，保证 Python 3.9+ 都能编译。
        delta_cell = delta.replace('|', '\\|')
        lines.append(f"| `{branch}` | {num} | {delta_cell} |")
...
def _claim_tree_card_text(
    ...
    *,
    ...
    for branch, num, delta in rows[:14]:
        # 同上：反斜杠不能出现在 f-string 表达式内（PEP 701 / Python 3.12+）
        delta_cell = delta.replace('|', '\\|')
        lines.append(f"| `{branch}` | {num} | {delta_cell} |")
    if len(rows) > 14:
