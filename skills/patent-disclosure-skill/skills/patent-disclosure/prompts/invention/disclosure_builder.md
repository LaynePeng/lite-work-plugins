...
### 通用技术语言与不造词（硬性，先于本章各条）

技术特征的措辞**尽量使用本领域通用技术术语**：连接、固定、夹持、检测、处理单元、通信接口、定时器等常规词直接可用，不加工、不修饰。**禁止随意生造词汇**——例如把「夹持」写成「环抱式定位握持」、把「连接器」写成「插拔耦合组件」、把「定时器」写成「时序锚定器」，都属于造词，即使「更形象」也要避免。

- **新概念怎么办**：用通用词**组合**表达，并在首次出现处加括号说明（白话机制），例如「实际使用资料清单（按实际读过的资料名称与版本登记）」；**不要**为表达新概念发明一个没见过的名词。
- **与「话题错位检测」的关系**：两条并列都要守——换领域词是换词，造新词是生造，前者要查，后者直接禁止。上文的领域可读名优先，不构成造新词的理由。
