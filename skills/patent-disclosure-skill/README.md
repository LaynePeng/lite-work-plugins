...
> **本仓库说明（lite-work 社区移植）**：本技能移植自
> [handsomestWei/patent-disclosure-skill](https://github.com/handsomestWei/patent-disclosure-skill)
> （MIT License，Copyright (c) 2026 handsomestWei，见 `LICENSE`）。
> 本地适配仅 3 处，**其余内容与上游 v4.7.0 一致**：
> 1. `skills/patent-reader/tools/vault/obsidian_claims.py` 两处 f-string 表达式内的反斜杠
>    改写（该写法仅 Python 3.12+ 合法，PEP 701）→ 现 **Python 3.9+ 全部可编译**；
> 2. `SKILL.md` 补 `triggers`，并在正文顶部加「技能目录 / 产出位置 / 前置条件与降级」说明；
> 3. 移除上游的仓库维护文件（`.github/`、`scripts/generate-star-history.py`），与技能功能无关。
>
> 外部依赖（TeX 无关，本技能不用 LaTeX）：**Playwright + 浏览器**（mermaid 出图 / CNIPA 检索）、
> **Obsidian**（解读 / 地图 / 审查答复）、**CadQuery**（STEP 三维视图，可选）。
> 缺失时上游已内建降级，不中断交付。
