...
def build_parser():
    parser = argparse.ArgumentParser(
        ...
        description='Chapter quality check: template / evaluate from JSON / compare chapters.')
    parser.add_argument('--template', nargs='?', const='-', metavar='OUT.json',
                        help='write a blank evaluation template JSON (no value = stdout)')
    parser.add_argument('--chapter-id', default='chapter_1', help='chapter id for --template')
    parser.add_argument('--chapter-title', default='', help='chapter title for --template')
    parser.add_argument('--word-count', type=int, default=0, help='target word count for --template')
    parser.add_argument('--json', metavar='FILE', help='evaluation JSON to score and report')
    parser.add_argument('--compare', metavar='FILE', help='list of chapter evaluations to compare')
...
def NON_INTERACTIVE_FLAGS(args):
    return args.template is not None or bool(args.json) or bool(args.compare)


def run_cli(args):
    if args.template is not None:
        template = create_chapter_evaluation_template()
        template['chapter_id'] = args.chapter_id
        template['chapter_title'] = args.chapter_title
        template['target_word_count'] = args.word_count
    ...
    if args.compare:
        chapters = json.loads(pathlib.Path(args.compare).read_text(encoding='utf-8'))
        _emit(compare_chapters(chapters), args.out)
        return 0

    if args.json:
        evaluation = json.loads(pathlib.Path(args.json).read_text(encoding='utf-8'))
        validation = validate_evaluation(evaluation)
        for issue in validation['issues']:
        ...
        if not validation['valid']:
            print('validation failed; report not generated', file=sys.stderr)
            return 1
        _emit(generate_chapter_report(evaluation), args.out)
        return 0
