...
def build_parser():
    parser = argparse.ArgumentParser(
        ...
        description='Final manuscript evaluation: template / evaluate from JSON.')
    parser.add_argument('--template', nargs='?', const='-', metavar='OUT.json',
                        help='write a blank evaluation template JSON (no value = stdout)')
    parser.add_argument('--title', default='', help='paper title for --template')
    parser.add_argument('--word-count', type=int, default=0, help='total word count for --template')
    parser.add_argument('--platform', default='', help='target platform for --template')
...
def NON_INTERACTIVE_FLAGS(args):
    return args.template is not None or bool(args.json)


def run_cli(args):
    if args.template is not None:
        template = create_final_evaluation_template()
        template['paper_title'] = args.title
        template['total_word_count'] = args.word_count
        template['target_platform'] = args.platform
        template['evaluation_date'] = datetime.now().strftime('%Y-%m-%d')
    ...
    if args.json:
        ...
        if not validation['valid']:
            ...
            return 1
        _emit(generate_final_report(evaluation), args.out)
        return 0
