...
triggers: write the paper,compose the manuscript,execute the outline,draft the paper,chapter quality,final evaluation,manuscript,submission ready,academic writing,paper composition,write from outline,polish paper
version: "1.0.0"
---
...
2. **Load reference documents**:
   ...
   - Section writing guides (`${SKILL_DIR}/references/section_guides.md`)
   - Quality standards (`${SKILL_DIR}/references/writing_standards.md`)
...
3. **Check section guide**:
   - Load appropriate template from `${SKILL_DIR}/references/section_guides.md`
...
1. **Create evaluation document**:
   ```bash
   python3 "${SKILL_DIR}/scripts/chapter_quality_check.py" --json <evaluation.json> --out <report.md>
#   other modes:  --template <out.json> [--chapter-id ...] | --compare <chapters.json>
...
```bash
python3 "${SKILL_DIR}/scripts/final_evaluation.py" --json <evaluation.json> --out <report.md>
#   other mode:   --template <out.json> [--title ... --word-count ... --platform ...]
...
```markdown
${SKILL_DIR}/references/writing_standards.md
```
...
```markdown
${SKILL_DIR}/references/section_guides.md
```
...
(`composer`), MIT License, Copyright (c) 2025 Li Shixiong — see `LICENSE.txt`.
