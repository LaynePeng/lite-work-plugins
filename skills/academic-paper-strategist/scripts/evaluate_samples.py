...
def build_parser():
    parser = argparse.ArgumentParser(
        prog='evaluate_samples.py',
        description='Sample-paper evaluation: template / report from JSON.')
    parser.add_argument('--template', nargs='?', const='-', metavar='OUT.json',
                        help='write a blank papers template JSON (no value = stdout)')
    parser.add_argument('--count', type=int, default=8, help='number of papers for --template (default 8)')
    parser.add_argument('--json', metavar='FILE', help='papers JSON to evaluate and report')
...
def run_cli(args):
    if args.template is not None:
        papers = [create_evaluation_template() for _ in range(max(1, args.count))]
    ...
    if args.json:
        papers = json.loads(pathlib.Path(args.json).read_text(encoding='utf-8'))
        _emit(generate_evaluation_report(papers), args.out)
        return 0
