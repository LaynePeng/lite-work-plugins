...
# ─────────────────────────────────────────────────────────────────────────────
# 非交互式 CLI（供 Agent / CI 调用）
# 无参数时：TTY 下进入交互式菜单；无 TTY（Agent/子进程）下打印用法并以 2 退出，
# 避免直接 input() 触发 EOFError。
# ─────────────────────────────────────────────────────────────────────────────
...
def build_parser():
    parser = argparse.ArgumentParser(
        prog='gap_analysis.py',
        description='Research gap analysis: template / validate one gap / portfolio report / evidence package.')
    parser.add_argument('--template', nargs='?', const='-', metavar='OUT.json',
                        help='write a blank gaps template JSON (no value = stdout)')
    parser.add_argument('--count', type=int, default=3, help='number of gaps for --template (default 3)')
    parser.add_argument('--validate', metavar='FILE', help='validate a single gap JSON')
    parser.add_argument('--json', metavar='FILE', help='gaps JSON list -> portfolio report')
    parser.add_argument('--gap', metavar='FILE', help='single gap JSON -> evidence package')
    parser.add_argument('--out', metavar='FILE', help='write output to FILE (default stdout)')
    parser.add_argument('--interactive', action='store_true', help='force the interactive menu')
    return parser


def NON_INTERACTIVE_FLAGS(args):
    return (args.template is not None or bool(args.validate)
            or bool(args.json) or bool(args.gap))


def run_cli(args):
    if args.template is not None:
        gaps = [create_gap_template() for _ in range(max(1, args.count))]
        for index, gap in enumerate(gaps, 1):
            gap['gap_id'] = f'GAP-{index:03d}'
        text = json.dumps(gaps, indent=2, ensure_ascii=False)
        if args.template == '-':
            print(text)
        else:
            pathlib.Path(args.template).write_text(text, encoding='utf-8')
            print(f'written: {args.template}')
        return 0

    if args.validate:
        gap = json.loads(pathlib.Path(args.validate).read_text(encoding='utf-8'))
        validation = validate_gap(gap)
        status = 'VALID' if validation['valid'] else 'INVALID'
        lines = [
            f"Gap: {gap.get('title', 'Untitled')}",
            f'Status: {status}',
            f"Evidence count: {validation['evidence_count']}",
            f"Definition length: {validation['definition_length']} words",
            f"'Why matters' length: {validation['why_matters_length']} words",
        ]
        for issue in validation['issues']:
    ...
    if args.json:
        gaps = json.loads(pathlib.Path(args.json).read_text(encoding='utf-8'))
        _emit(generate_gap_report(gaps), args.out)
        return 0

    if args.gap:
        gap = json.loads(pathlib.Path(args.gap).read_text(encoding='utf-8'))
        _emit(generate_gap_evidence_package(gap), args.out)
        return 0
...
def main(argv=None):
    ...
    if not NON_INTERACTIVE_FLAGS(args):
        if sys.stdin.isatty():
            return interactive_main()
        parser.print_help()
        print('\n(no TTY: interactive menu skipped; use the options above instead)',
              file=sys.stderr)
        return 2
    return run_cli(args)
