...
triggers: plan a paper,design a paper,research gaps,is this idea original,literature review,preprint,philarchive,arxiv,philsci-archive,paper outline,research planning,originality,strategic planning,gap analysis
version: "1.0.0"
---
...
> **Skill directory (`${SKILL_DIR}`)**: the folder this file lives in. All paths below are
> relative to it — read reference docs as `${SKILL_DIR}/references/<file>.md` and run tools as
> `python3 "${SKILL_DIR}/scripts/<tool>.py"`.
>
> **Scripts are non-interactive.** Always pass CLI flags (see `--help` on each script).
> With no arguments a script needs a TTY to show its menu; when run from an agent/CI
> context (no TTY) it prints usage and exits `2` instead of hanging on `input()`.
...
**Search Strategy** (load `${SKILL_DIR}/references/search_strategy.md` for details):
...
```bash
python3 "${SKILL_DIR}/scripts/evaluate_samples.py" --json <papers.json> --out <report.md>
#   other mode:   --template <out.json> [--count N]
```
...
**Load Reference**: `${SKILL_DIR}/references/search_strategy.md` for detailed methodology
...
```bash
python3 "${SKILL_DIR}/scripts/gap_analysis.py" --json <gaps.json> --out <report.md>
#   other modes:  --template <out.json> [--count N] | --validate <gap.json> | --gap <gap.json>
```
...
I will evaluate the outline as if I were a platform reviewer, using **7 dimensions** (load `${SKILL_DIR}/references/quality_standards.md` for criteria):
...
```markdown
${SKILL_DIR}/references/quality_standards.md
```
...
Ported from [lishix520/academic-paper-skills](https://github.com/lishix520/academic-paper-skills)
(`strategist`), MIT License, Copyright (c) 2025 Li Shixiong — see `LICENSE.txt`.

Adaptations made in this repository:

1. **Non-interactive CLI** added to every script (argparse). Upstream scripts only offered an
   interactive `input()` menu, which hangs or raises `EOFError` in an agent/subprocess context
   with no TTY. All analysis functions are reused unchanged; `--interactive` still opens the
   original menu.
2. **`gap_analysis.py` syntax fixed**: it used nested same-quote f-strings
   (`f"Status: {{"OK" if ...}}"`), which is only legal on Python 3.12+ and made the file fail
   with `SyntaxError` on older interpreters. Now works on Python 3.9+.
3. **Paths made `${SKILL_DIR}`-relative**, and `python` → `python3` (macOS has no bare `python`).
4. **Frontmatter extended** with `triggers` and top-level `version` so the host can index the
   skill and detect community updates (upstream shipped only `name` + `description`).
5. Report/summary output goes wherever you point `--out` (use the workspace `产出物/` directory),
   instead of being written next to the input JSON.
