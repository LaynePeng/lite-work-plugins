# 上游参考正文（docx-editor-cn / REFERENCE）

> 本文件为上游 `docx-editor-cn` 的 SKILL.md 正文**原样保留**，供深入查阅：
> 创建与编辑文档的完整流程、XML 参考（修订留痕 / 批注 / 图片）、Markdown 转换的
> 11 个已解决问题（Issues 1–11）、MathML→DOCX 映射等。
>
> 阅读时注意：文中形如 `scripts/xxx.py` 的路径均**相对于技能目录** `${SKILL_DIR}`；
> `python scripts/office/*.py` 需先 `cd ${SKILL_DIR}/scripts/office` 再运行。
> 日常使用请看 `SKILL.md`。
